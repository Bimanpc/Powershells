...
.DESCRIPTION
    Device Registration Troubleshooter Tool is an  script that troubleshoots device registration common issues.
...
Function Test-DevRegConnectivity($Write){
    ...
    $TestConnResult=@()
    If($Write){Write-Host}
    If($Write){Write-Host "Testsss Internet Connectivity..." -ForegroundColor Yellow;  Write-Log -Message "Testing Internet Connectivity..."}
    $ErrorActionPreference= 'silentlycontinue'
    $global:TestFailed=$false

    $global:ProxyServer = checkProxy $Write
    If($Write){Write-Host}
    If($Write){Write-Host "Testssss Device Registration Endpoints..." -ForegroundColor Yellow; Write-Log -Message "Testing Device Registration Endpoints..."}
    if ($global:ProxyServer -ne "NoProxy"){
        If($Write){Write-Host "Testsssss connection via winHTTP proxy..." -ForegroundColor Yellow; Write-Log -Message "Testing connection via winHTTP proxy..."}
        if ($global:login){
...
cls
'==========================================================='
Write-Host '          Device Registration Troubleshooter Tool By Vasiya         ' -ForegroundColor Green 
'==========================================================='
Write-Host ''
